...
//FSMD states
parameter S1 = 4'h01, S2 = 4'h02, S3 = 4'h03, S4 = 4'h04, S5 = 4'h05, S6 = 4'h06,
          S7 = 4'h07, S8 = 4'h08, S9 = 4'h09, S10 = 4'h0a, S11 = 4'h0b, S12 = 4'h0c,
          S13 = 4'h0d;
reg [3:0] state;
    ...
    begin
        case(state)
            ...
            S3: begin
                sin_frac <= sin >>> i;
                cos_frac <= cos >>> i;
                d <= 10;                 // ensure d < i is false
                atan_val <= atan[i];
                state <= S4;
            end
            S4: begin
                // ignore the d<i condition; it is always false because d=10
                // direction decision based on current angle vs target
                if(angle < t_angle)
                    state <= S6;
                else
                    state <= S7;
            ...
            S5: begin
                // This state is not used in the normal flow.
                // Provide dummy assignments to avoid latches.
                sin_frac <= sin_frac >>> 1;
                cos_frac <= cos_frac >>> 1;
                d <= 0;
                state <= S6;
